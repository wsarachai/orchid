...
def batch_creator(batch_size, dataset_length, dataset_name):
    ...
    batch_x = eval(dataset_name + '_x')[[batch_mask]].reshape(-1, 32*32*3)
...
def deepnn(x):
    ...
    with tf.name_scope('fullyc_1') as scope:
    ...
    with tf.name_scope('fullyc_2') as scope:
...
def feed_dict(train):
  if train or FLAGS.fake_data:
      batch_x, batch_y = batch_creator(FLAGS.batch_size, train_x.shape[0], 'train')
      k = FLAGS.dropout
  else:
      batch_x, batch_y = batch_creator(FLAGS.batch_size, train_x.shape[0], 'test')
      k = 1.0
  return {x: batch_x, y: batch_y, keep_prob: k}
...
output_layer, keep_prob = deepnn(x)
...
with tf.Session() as sess:
    ...
    for step in range(FLAGS.epochs):
        ...
        for i in range(total_batch):
            batch_x, batch_y = batch_creator(FLAGS.batch_size, train_x.shape[0], 'train')
            #_, c = sess.run([optimizer, cross_entropy], feed_dict=feed_dict(True))
            c = sess.run(optimizer, feed_dict=feed_dict(True))

            avg_cost += c / total_batch

        print ("Epoch:", (step + 1), "cost =", "{:.5f}".format(avg_cost))
    print ("\nTraining complete!")
    ...
    print ("Validation Accuracy:", accuracy.eval(feed_dict=feed_dict(True)))
    #predict = tf.argmax(output_layer, 1)
    #pred = predict.eval({x: test_x.reshape(-1, 32*32*3)})